...
void main() {
	...
	if (chandelierIntensity > 0.0) {
		...
		vec3 l_bulbCol = chandelierColor * pow(chandelierTarget / length(chandelierPosition - fs_position), chandelierDecay);
		vec4 bulbLambert = lambert(l_chandelierDir, normal, texColor);
		lightSum = lightSum + lightComponent(bulbLambert, l_bulbCol, chandelierIntensity);
